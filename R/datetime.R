...
date_to_seasonname <- function(x) UseMethod("date_to_seasonname")
#' @describeIn date_to_seasonname non POSIXct or Date classes
date_to_seasonname.default <- function(x,
  lut = c(
    'DJF', 'DJF',
    'MAM', 'MAM', 'MAM',
    'JJA', 'JJA', 'JJA',
  ...
  if (missing(x)) {
    x <- Sys.Date()
    return(date_to_seasonname.Date(x))
  } else {
    stop("no functionality available for class: ", paste(class(x), collapse = " "))
  }
}
#' @describeIn date_to_seasonname for POSIXct classes
date_to_seasonname.POSIXct <- function(x,
  lut = c(
      'DJF', 'DJF',
      'MAM', 'MAM', 'MAM',
      'JJA', 'JJA', 'JJA',
...
#' @describeIn date_to_seasonname for Date classes
date_to_seasonname.Date <- function(x,
  lut = c(
    'DJF', 'DJF',
    'MAM', 'MAM', 'MAM',
    'JJA', 'JJA', 'JJA',
...
date_to_seasonstart <- function(x) UseMethod("date_to_seasonstart")
#' @describeIn date_to_seasonstart for missing input
date_to_seasonstart.default <- function(x){
  if (missing(x)) {
    x <- Sys.Date()
    return(date_to_seasonstart.Date(x))
  } else {
    stop("no functionality available for class: ", paste(class(x), collapse = " "))
  }
}
#' @describeIn date_to_seasonstart for POSIXct input
date_to_seasonstart.POSIXct <- function(x){
    r <- range(x)
    s <- seq_season(from = r[1], to = r[2])
    s[findInterval(x, s)]
}
#' @describeIn date_to_seasonstart for Date input
date_to_seasonstart.Date <- function(x){
  r <- range(x)
  s <- seq_season(from = r[1], to = r[2])
  s[findInterval(x, s)]
}
...
date_to_week <- function(x) UseMethod("date_to_week")
#' @describeIn date_to_week for missing input
date_to_week.default <- function(x, week_length = c(8,7)[1]){
  if (missing(x)) {
    x <- Sys.Date()
    return(date_to_week.Date(x, week_length = week_length))
  } else {
    stop("no functionality available for class: ", paste(class(x), collapse = " "))
  }
}
#' @describeIn date_to_week for POSIXct input
date_to_week.POSIXct <- function(x = Sys.time(), week_length = c(8,7)[1]){
    J <- as.numeric(format(x, "%j"))
    (J-1) %/% week_length + 1
}
#' @describeIn date_to_week for Date input
date_to_week.Date <- function(x = Sys.Date(), week_length = c(8,7)[1]){
  J <- as.numeric(format(x, "%j"))
  (J-1) %/% week_length + 1
}
...
#' }
date_to_seasonprior <- function(x) UseMethod("date_to_seasonprior")
#' @describeIn date_to_seasonprior for missing input
date_to_seasonprior.default <- function(x){
  if (missing(x)) {
    return(date_to_seasonprior.Date(x = Sys.Date()))
  } else {
    stop("no functionality available for class: ", paste(class(x), collapse = " "))
  }
}
#' @describeIn date_to_seasonprior for POSIXct input
date_to_seasonprior.POSIXct <- function(x = Sys.time()){
...
#' @describeIn date_to_seasonprior for Date input
date_to_seasonprior.Date <- function(x = Sys.Date()){
...
seq_season <- function(from, to) UseMethod("seq_season")
#' @describeIn seq_season for POSIXct input
seq_season.POSIXct <- function(
...
#' @describeIn seq_season
#'  for Date input
seq_season.Date <- function(
  from = as.Date("2003-12-01"),
  to = Sys.Date()){
  to <- as.Date(to)
  fy <- lubridate::year(from[1])
  fm <- lubridate::month(from[1])
...
#' @describeIn today for missing input
today.default <- function(x,
...
#' @describeIn today for POSIXct input
today.POSIXct <- function(x = as.POSIXct(paste(Sys.Date(), "00:00:00"),tz = 'UTC'),
...
#' @describeIn today for Date input
today.Date <- function(x = Sys.Date(),
  form = c('POSIXct', "Date", "%Y-%m-%d", 'week', 'season')[1]){
  switch(form[1],
         'week' = date_to_week(x),
         'season' = date_to_seasonstart(x),
...
tomorrow <- function(x) UseMethod("tomorrow")
#' @describeIn tomorrow for missing input
tomorrow.default <- function(x, n = 1){
  if (missing(x)) x <- Sys.Date()
  x + n
}
#' @describeIn tomorrow for POSIXct input
tomorrow.POSIXct <- function(x = Sys.time(), n = 1){
   x + (n * 86400)
}
#' @describeIn tomorrow for Date input
tomorrow.Date <- function(x = Sys.Date(), n = 1){
  x + n
}
...
#' @return POSIXct
yesterday <- function(x) UseMethod("yesterday")
#' @describeIn yesterday for missing input
yesterday.default <- function(x, n = -1){
  if (missing(x)) x <- Sys.Date()
  x + n
}
#' @describeIn yesterday for POSIXct input
yesterday.POSIXct <- function(x = Sys.time(), n = -1){
    x + (n * 86400)
}
#' @describeIn yesterday for Date input
yesterday.Date <- function(x = Sys.Date(), n = -1){
  x + n
}
